...
Route::accept($config->manager->slug . '/plugin/' . File::B(__DIR__) . '/update', function() use($config, $speak) {
    if($request = Request::post()) {
        ...
        $css = File::open(PLUGIN . DS . File::B(__DIR__) . DS . 'shell' . DS . 'pigment' . DS . $request['skin'] . '.css')->read();
        File::write(Converter::detractShell($css . "\n\n" . trim($request['css'])))->saveTo(PLUGIN . DS . File::B(__DIR__) . DS . 'shell' . DS . 'spoiler.css');
